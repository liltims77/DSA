...
# If l1 is not None:
# Just move forward → l1 = l1.next

# If l1 is None (reached end of road A):
# Jump to the start of road B → l1 = headB
...
# When a pointer reaches the end, we switch it:

# l1 switches to headB → starts at 4

# l2 switches to headA → starts at 1
# After switching roads, both people have walked the same total distance, so they will meet at the intersection node.
# Person 1: 1→2→3→7→8→9→NULL→4→5→7→8→9  
# Person 2: 4→5→7→8→9→NULL→1→2→3→7→8→9  
